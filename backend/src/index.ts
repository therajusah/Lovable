...
app.post('/prompt', async (req, res) => {
  ...
  try {
    ...
    const { sandbox, sandboxId, previewUrl } = await sandboxManager.createSandbox(
      TEMPLATE_ID,
    ...
    for await (const delta of textStream.fullStream) {
      if (delta.type === 'text-delta') {
        ...
        try {
          const result = await ToolHandlers.executeToolCall(
            delta.toolName,
            delta.input as any,
            toolContext
          );
          ...
          const errorMsg = `Error executing tool ${delta.toolName}: ${
