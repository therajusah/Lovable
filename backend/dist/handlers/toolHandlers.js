...
export class ToolHandlers {
    static async handleCreateFile(input, context) {
    ...
    static async handleUpdateFile(input, context) {
    ...
    static async handleDeleteFile(input, context) {
    ...
    static async handleReadFile(input, context) {
    ...
    static async handleRunCommand(input, context) {
        try {
            ...
            context.eventEmitter.emitCommandCompleted(input.command, stdout, stderr, execution.error);
            return result;
        }
        catch (cmdError) {
            const errorMessage = `Command failed: ${cmdError instanceof Error ? cmdError.message : String(cmdError)}`;
            context.eventEmitter.emitCommandError(input.command, errorMessage);
            return errorMessage;
        }
    }
    static async executeToolCall(toolName, input, context) {
        try {
            switch (toolName) {
                case 'createFile':
            ...
            const errorMsg = `Tool failed: ${error instanceof Error ? error.message : String(error)}`;
